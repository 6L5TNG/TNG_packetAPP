...
class SettingsDialog(QDialog):
    ...
    def _build_general_tab(self):
        ...
        r3 = QHBoxLayout(); r3.addWidget(QLabel(Translator.tr("lbl_theme"))); r3.addWidget(self.combo_theme); r3.addWidget(QLabel(Translator.tr("lbl_lang"))); r3.addWidget(self.combo_lang)
    ...
    def _build_transmit_tab(self):
        ...
        # Beep Settings (Restored)
        self.chk_beeps = QCheckBox(Translator.tr("lbl_beeps"))
        self.chk_beeps.setChecked(self.settings.get('enable_beeps', True))
        l_mpda.addWidget(self.chk_beeps)
        
        row1 = QHBoxLayout()
        self.txt_start_beep = QLineEdit(self.settings.get('start_beep', '250:400:250'))
        row1.addWidget(QLabel(Translator.tr("lbl_start_beep"))); row1.addWidget(self.txt_start_beep)
        l_mpda.addLayout(row1)
        
        row2 = QHBoxLayout()
        self.txt_end_beep = QLineEdit(self.settings.get('end_beep', '250:400:250'))
        row2.addWidget(QLabel(Translator.tr("lbl_end_beep"))); row2.addWidget(self.txt_end_beep)
        l_mpda.addLayout(row2)
    ...
    def _build_receive_tab(self):
        ...
        sub_mpda = QWidget(); self.sub_tabs_rx.addTab(sub_mpda, Translator.tr("sub_mpda")); l_mpda = QVBoxLayout(sub_mpda); l_mpda.addWidget(QLabel("MPDA RX Thresholds")); l_mpda.addStretch()
        sub_chirp = QWidget(); self.sub_tabs_rx.addTab(sub_chirp, Translator.tr("sub_chirp")); l_chirp = QVBoxLayout(sub_chirp); l_chirp.addWidget(QLabel("TNG_Chirp RX Settings")); l_chirp.addStretch()
        sub_wf = QWidget(); self.sub_tabs_rx.addTab(sub_wf, Translator.tr("sub_wf")); l_wf = QVBoxLayout(sub_wf)
        ...
        sub_spec = QWidget(); self.sub_tabs_rx.addTab(sub_spec, Translator.tr("sub_spec")); l_spec = QVBoxLayout(sub_spec); l_spec.addWidget(QLabel("Spectrum Settings")); l_spec.addStretch()
    ...
    def _populate_audio_devices(self):
        try:
            ...
            for i, d in enumerate(devices):
                if mme_index != -1 and d['hostapi'] != mme_index: continue
                name = d['name']
                if d['max_input_channels'] > 0: input_devs.append(f"{i}: {name}")
                if d['max_output_channels'] > 0: output_devs.append(f"{i}: {name}")
            self.combo_in.addItems(input_devs if input_devs else ["No MME Devices"])
            self.combo_out.addItems(output_devs if output_devs else ["No MME Devices"])
            saved_in = self.settings.get('audio_in'); saved_out = self.settings.get('audio_out')
            if saved_in:
